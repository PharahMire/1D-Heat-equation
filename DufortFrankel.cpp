...
DufortFrankel::DufortFrankel(double dt, double dx, double L, double D, double T_sur, double Tin, double tmax) :
...
void DufortFrankel::compute() {
    ...
    // Compute DuFort-Frankel algorithme method
    for (int j = 2; j < nTimeNodes; j++) {
        for (int i = 1; i < nSpaceNodes - 1; i++) {
            fp2[i] = (fp[i] + 2 * r * (fp1[i + 1] - fp[i] + fp1[i - 1])) / (1 + 2 * r);
        }
        fp = fp1;
    ...
    print(data, "Dufort-Frankel.csv", dx);
    return;
}
